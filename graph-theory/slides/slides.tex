...
\begin{definition}[Inducted Subgraph]
	If $G = (V, E)$ is a graph and $X \subseteq V$, the \vocab{subgraph inducted by $X$} is defined to be $G[X] = (X, \{xy \in E : x, y \in X\})$.
\end{definition}
...
We can define the neighborhood of a subset of vertices in a similar way.
\end{frame}
...
\begin{frame}
  ...
  \begin{definition}[Minimum/Maximum Degree]
    Let $G$ be a graph. The \vocab{maximum degree} of $G$, $\Delta(G)$ is defined to be 
    $$\Delta(G) = \max_{x \in V} d(x).$$
    Similarly, we define the \vocab{minimum degree} of $G$, $\delta(G)$ to be 
...
% where a vertex $u$ is connected to vertex $v$ if you can follow some path in the graph to get from $u$ to $v$.
...
For example, in the graph above we want to say somehow that $u$ and $v$ are connected, but $u$ and $v'$ are not.
...
\begin{definition}[Connectivity]
  In a graph $G = (V, E)$, we say that two vertices $u, v \in V$ are \vocab{connected} if there is some path between $u$ and $b$ using only edges in $E$.
\end{definition}
...
\begin{definition}[Connected Graph]
  ...
  \begin{definition}[Connected Components]
      A \vocab{component} of a graph $G$ is a subgraph in which any two vertices in the subgraph are connected, but no vertex in the subgraph is connected to a vertex outside of the subgraph.
  \end{definition}
...
\begin{frame}
  ...
  \begin{center}
    \includegraphics[width=0.75\textwidth]{trees.jpg}
  \end{center}
...
\draw  [draw opacity=0][fill={rgb, 255:red, 208; green, 2; blue, 27 }  ,fill opacity=1 ] (250,165) .. controls (250,162.24) and (252.24,160) .. (255,160) .. controls (257.76,160) and (260,162.24) .. (260,165) .. controls (260,167.76) and (257.76,170) .. (255,170) .. controls (252.24,170) and (250,167.76) .. (250,165) -- cycle ;
%Shape: Circle [id:dp922502556460108] 
\draw  [draw opacity=0][fill={rgb, 255:red, 208; green, 2; blue, 27 }  ,fill opacity=1 ] (200,215) .. controls (200,212.24) and (202.24,210) .. (205,210) .. controls (207.76,210) and (210,212.24) .. (210,215) .. controls (210,217.76) and (207.76,220) .. (205,220) .. controls (202.24,220) and (200,217.76) .. (200,215) -- cycle ;
%Shape: Circle [id:dp19932072180727434] 
\draw  [draw opacity=0][fill={rgb, 255:red, 208; green, 2; blue, 27 }  ,fill opacity=1 ] (230,215) .. controls (230,212.24) and (232.24,210) .. (235,210) .. controls (237.76,210) and (240,212.24) .. (240,215) .. controls (240,217.76) and (237.76,220) .. (235,220) .. controls (232.24,220) and (230,217.76) .. (230,215) -- cycle ;
%Shape: Circle [id:dp2630213009599035] 
\draw  [draw opacity=0][fill={rgb, 255:red, 208; green, 2; blue, 27 }  ,fill opacity=1 ] (260,215) .. controls (260,212.24) and (262.24,210) .. (265,210) .. controls (267.76,210) and (270,212.24) .. (270,215) .. controls (270,217.76) and (267.76,220) .. (265,220) .. controls (262.24,220) and (260,217.76) .. (260,215) -- cycle ;
%Shape: Circle [id:dp9778477086384065] 
\draw  [draw opacity=0][fill={rgb, 255:red, 208; green, 2; blue, 27 }  ,fill opacity=1 ] (320,215) .. controls (320,212.24) and (322.24,210) .. (325,210) .. controls (327.76,210) and (330,212.24) .. (330,215) .. controls (330,217.76) and (327.76,220) .. (325,220) .. controls (322.24,220) and (320,217.76) .. (320,215) -- cycle ;
%Straight Lines [id:da4037746415210903] 
\draw  [dash pattern={on 0.84pt off 2.51pt}]  (288,215) -- (300,215) ;
...
\begin{frame}
  ...
  In general, trees have leaves.

\begin{proposition}[Trees Have Leaves]
	Every tree $T$ with $|T| \geq 2$ has a leaf.
\end{proposition}
\pause
\emph{Proof}. Let $P$ be a path of maximum length in $T$, with $P = x_1 x_2 \dots x_k$. We claim $\deg(x_k) = 1$. Suppose $x_k$ was adjacent to some vertex $y$ other than $x_{k - 1}$. Then if $y$ already occurred in the path the graph would contain a cycle which is a contradiction, and otherwise if $y$ did not occur in the path, it could be added to it, increasing the length and contradicting maximality. Thus there can be no other vertex adjacent to $x_k$.


\vspace{\baselineskip}

\textbf{Remark}. This proof gives us two leaves in $T$, which is the best we can hope for considering $P_n$ is a tree with exactly two leaves.
...
\begin{frame}
  ...
  \emph{Proof}. Left as an exercise.%\footnote{If you want a hint, try use induction.}.
...
\begin{frame}
  ...
  \begin{definition}[Spanning Tree]
    Let $G = (V, E)$ be a graph. We say $T$ is a \vocab{spanning tree} of $G$ if $T$ is a tree on $V$ and is a subgraph of $G$.
  \end{definition}
...
Spanning trees are useful in lots of contexts, one of which is giving a sensible ordering to the vertices of a graph. They are particularly useful because of the following result.
...
\begin{frame}
  ...
  \begin{example}[Veblen's Theorem]
...
\begin{frame}
  ...
  \begin{example}[Euler's Formula]
    Let $G$ be a connected planar\footnote{We say that a graph is \vocab{planar} if it can be drawn on the plane $\mathbb{R}^2$ that no edges overlap. We will also call the connected components of $\mathbb{R}^2 - G$ the \vocab{faces} of the graph.} graph with $V$ vertices, $E$ edges and $F$ faces. Then
    $$
    V - E + F = 2.
    $$
    \end{example}

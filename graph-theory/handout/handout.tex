...
Graph theory is an area of combinatorics which has lots of fun problems and plenty of interesting theorems. It's a common topic in Olympiads and is a very active field of research. In this handout we are going to look at the basics of Graph Theory, with a particular focus on how it's used in solving Olympiad problems. 
...
\begin{remark}
	In our definition of a graph, we {\itshape don't allow}\footnote{These limitations are inherent in our definition, where we use sets rather than multisets.} loops, and there {\itshape cannot} be multiple edges between the same set of vertices.
	\begin{center}
...
% Now that we have defined graphs, it's natural to define some notion of when they are the same.
% It's important to note that how we draw a graph doesn't really matter -- it will not change the underlying graph. Indeed, we usually say that two graphs are the same (or are \emph{isomorphic}) if 
...
\subsubsection{Neighbors and Degree}
...
For example, in the graph above we want to say somehow that $u$ and $v$ are connected, but $u$ and $v'$ are not. With this in mind, we can introduce some more definitions.

\begin{definition}[Connectivity]
    In a graph $G = (V, E)$, we say that two vertices $u, v \in V$ are \vocab{connectied} if there is some path between $u$ and $b$ using only edges in $E$.
\end{definition}
...
\end{proposition}
\begin{proof}
    We will prove that (a) implies (b), and leave the rest as an exercise.

    By definition $G = (V, E)$ is acyclic and connected. So let $x, y \in V$ such that $xy \not \in E$. As $G$ is connected, there is some path between $x$ and $y$ in the graph, so then adding the edge $xy$ to this path gives us a cycle.
\end{proof}
% \begin{proof}
...
\end{proposition}
\begin{proof}
    Left as an exercise\footnote{If you want a hint, try use induction.}.
...
\subsection{Problem Solving in Graph Theory}
...
One strategy you should always have to hand is using \emph{induction}. Induction works unusually well for proving facts in graph theoretic settings, though it's not always obvious how it should be applied. Usually inductive proofs in graph theory look like:
...
Let $G$ be a connected planar\footnote{We say that a graph is \vocab{planar} if it can be drawn on the plane $\R^2$ that no edges overlap. We will also call the connected components of $\R^2 - G$ the \vocab{faces} of the graph.} graph with $V$ vertices, $E$ edges and $F$ faces. Then
$$
V - E + F = 2.
$$
\end{example}
...
Of course, as with most problems in Combinatorics, you will need to be creative (which is what makes the problems fun!)
